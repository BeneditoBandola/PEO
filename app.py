...
# ==========================================
# 1. DOWNLOAD DOS DADOS DO PDV PET (PLAYWRIGHT)
# ==========================================
def baixar_dados_pdvpet():
    st.write("--- INICIANDO DOWNLOAD DO HISTÓRICO (P9 e P10) VIA PLAYWRIGHT ---")
    ...
    try:
        with sync_playwright() as p:
            ...
            page.goto("https://www.pdvpet.com.br/", timeout=60000)
            ...
            page.click("text=/questionários/i")
            ...
            with page.expect_download(timeout=60000) as download_info:
                page.click("button.btn-outline-success")
            
            download = download_info.value
            download.save_as(CAMINHO_CSV_FINAL)
...
# ==========================================
def enviar_email(filial, caminho_pdf, tem_preco_pendente, apenas_meus_emails=False):
    ...
    if apenas_meus_emails:
        destinatarios = list(set(EMAILS_MEUS))
    else:
    ...
    try:
        ...
        server.quit()
        st.success(f"📧 E-mail enviado com sucesso para '{filial}': {destinatarios}")
    except Exception as e:
...
def processar_e_gerar_relatorios(apenas_meus_emails=False, forcar_download=True, tipo_relatorio="Completo (Oportunidades + Preços)"):
    ...
    st.write("--- PROCESSANDO DADOS E GERANDO RELATÓRIOS ---")
...
# ==========================================
st.set_page_config(page_title="Painel PDV Pet - P10", page_icon="📊", layout="wide")

st.title("📊 Painel de Automação PDV Pet")
st.caption(f"Período Vigente: **{PERIODO_ATUAL}** | Comparação: **P9** ({INICIO_P9} a {FIM_P9}) vs **P10** ({INICIO_P10} a {FIM_P10})")
...
modo_envio = st.sidebar.radio(
    ...
    options=["Apenas Benedito", "Todos (Benedito + Promotores)"],
    index=0
)
...
forcar_download = st.sidebar.checkbox(
    ...
    value=True,
    help="Se desmarcado e já existir um CSV local, usará o arquivo existente."
)

st.sidebar.markdown("---")
st.sidebar.write("**E-mails Cadastrados (Benedito):**")
...
with col2:
    st.subheader("🏢 Filiais Mapeadas")
    for f in METAS.keys():
        st.write(f"• {f}")

st.markdown("---")

if st.button("🚀 Iniciar Processamento e Envio", type="primary", use_container_width=True):
    apenas_benedito = (modo_envio == "Apenas Benedito")
    
    st.info("🔄 Processo iniciado. Acompanhe os logs abaixo...")
